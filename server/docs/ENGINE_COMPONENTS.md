...
A transport-independent generation channel (bounded request queue plus
consumer/producer endpoints for token batches, progress, and the terminal
result) is the intended next step for request submission; it is not part of
this change and lands with its HTTP caller.
