...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    static const bool destination_stream_copies_enabled = [] {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        bool split_copy_source_ready[GGML_SCHED_MAX_BACKENDS] = {};
        auto wait_for_split_copy_generation = [&](ggml_backend_t input_backend,
                                                   int input_backend_id) {
            if (sched->batch_split_copies && split_copy_generation_ready) {
                return;
            }

            ggml_backend_event_t generation_event =
                sched->events[split_backend_id][sched->cur_copy];
            if (sched->single_copy_event_fences && generation_event != NULL &&
                input_backend_id != split_backend_id &&
                input_backend->iface.event_wait != NULL) {
                // Async copies run after backend_src, while backend_dst waits
                // for their completion. Put the missing destination-buffer
                // lifetime edge on the source stream:
                //
                // dst previous use -> src copy -> dst next use.
                //
                // A split can have inputs from multiple backends, so one wait
                // is required per source stream rather than merely per split.
                if (!sched->batch_split_copies ||
                    !split_copy_source_ready[input_backend_id]) {
                    ggml_backend_event_wait(input_backend, generation_event);
                    split_copy_source_ready[input_backend_id] = true;
                }
                return;
            }

            if (generation_event != NULL) {
                ggml_backend_event_wait(
                    split_backend, generation_event);
            } else {
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            GGML_ASSERT(input_backend_id >= 0 &&
                        input_backend_id < sched->n_backends);
            ggml_backend_t input_backend = sched->backends[input_backend_id];
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                // A destination-stream peer copy is naturally ordered after
                // every prior use of its scheduler buffer on that stream. Do
                // not add the ordinary source-stream lifetime fence: doing so
                // makes the producer wait for the peer and destroys the fork
                // overlap this explicitly marked staging path exists to gain.
                //
                // The flag is only attached when the HIP destination-stream
                // implementation is enabled. Other copies retain the normal
                // generation fence below.
                const bool destination_stream_copy =
                    (input->flags & GGML_TENSOR_FLAG_DST_STREAM_COPY) != 0 &&
                    destination_stream_copies_enabled;
                if (!destination_stream_copy) {
                    // Wait for the split backend to finish using the input
                    // buffer before a source-stream copy overwrites it.
        ...
        if (split->n_inputs > 0 || sched->single_copy_event_fences) {
            if (sched->events[split_backend_id][sched->cur_copy] != NULL) {
...
void ggml_backend_sched_free(ggml_backend_sched_t sched) {
    ...
    // graph_compute_async callers and native graph caches may still hold
    // queued references to scheduler events or gallocr buffers. Teardown is
    // rare (cache eviction/shutdown), so quiesce every backend before freeing
    // any object that a device stream can still reference.
    for (int b = 0; b < sched->n_backends; ++b) {
...
void ggml_backend_sched_set_single_copy_event_fences(
        ggml_backend_sched_t sched, bool enabled) {
    GGML_ASSERT(sched);
    GGML_ASSERT(!sched->is_alloc);
    GGML_ASSERT(sched->n_copies == 1);

    if (enabled == sched->single_copy_event_fences) {
        return;
    }

    if (enabled) {
        for (int b = 0; b < sched->n_backends; ++b) {
            GGML_ASSERT(sched->events[b][0] == NULL);
            sched->events[b][0] =
                ggml_backend_event_new(sched->backends[b]->device);
            if (sched->events[b][0] != NULL) {
                // Give the first source-side wait a defined generation and
                // include work queued before scheduler creation.
                ggml_backend_event_record(
                    sched->events[b][0], sched->backends[b]);
            }
        }
    } else {
        // A reset can make is_alloc false while a prior submission is still
        // running. Finish it before destroying events referenced by streams.
        ggml_backend_sched_synchronize(sched);
        for (int b = 0; b < sched->n_backends; ++b) {
            ggml_backend_event_free(sched->events[b][0]);
            sched->events[b][0] = NULL;
        }
    }

    sched->single_copy_event_fences = enabled;
}
