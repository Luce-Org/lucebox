...
#define MMQ_DP4A_TXS_ROCMFPX tile_x_sizes{mmq_y*MMQ_TILE_NE_K*2 + mmq_y, mmq_y*MMQ_TILE_NE_K/2 + mmq_y/2,             0}
...
#define MMQ_MMA_TILE_X_K_ROCMFPX (2*MMQ_TILE_NE_K + MMQ_TILE_NE_K/2                       + 4)
...
static_assert(MMQ_MMA_TILE_X_K_ROCMFPX % 8 == 4, "Wrong padding.");
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q2_0_ROCMFP2:
        case GGML_TYPE_Q3_0_ROCMFPX:
                                 return MMQ_MMA_TILE_X_K_ROCMFPX;
...
// ROCmFP4_FAST has one UE4M3 scale per 32-weight block. Its expanded int8
// tile is therefore layout-compatible with Q8_0 MMQ after Codebook10 decode.
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfp4_fast(
    ...
    constexpr int words_per_block = QK_ROCMFP4 / 4;
    constexpr int threads_per_row = MMQ_ITER_K / 8;
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
...
#pragma unroll
        for (int part = 0; part < 2; ++part) {
            const int word = part*MMQ_TILE_NE_K + txi;
            const int block = word / words_per_block;
            const int word_in_block = word % words_per_block;
            const block_rocmfp4_fast * bxi = (const block_rocmfp4_fast *) x + kbx0 + i*stride + block;
            const int2 values = rocmfp4_get_int_from_codebook_16(
                rocmfp4_get_qs_i32(bxi->qs, word_in_block % 4), kvalues_rocmfp4);
            const int packed = word_in_block < 4 ? values.x : values.y;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + word] = packed;
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + word] = packed;
#endif
        }

        if (txi < 2*MMQ_TILE_NE_K/QI8_0) {
            const block_rocmfp4_fast * bxi = (const block_rocmfp4_fast *) x + kbx0 + i*stride + txi;
            const float scale = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e);

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_df[i*MMQ_MMA_TILE_X_K_Q8_0 + txi] = scale;
#else
            x_df[i*(2*MMQ_TILE_NE_K/QI8_0) + i/(QI8_0/2) + txi] = scale;
#endif
        }
    }
}

// ROCmFP2/FP3 use two UE4M3 scales per 32-value block. Expand each packed
// block into the same int8 tile shape as Q8_0, but retain one scale per
// 16-value half-block so the DP4A reduction remains numerically exact.
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfp2(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_qs + 2*MMQ_TILE_NE_K);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q2_0_ROCMFP2, mmq_y);
    int   * x_qs = (int *) x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif

    constexpr int words_per_block = QK_ROCMFP2 / 4;
    constexpr int threads_per_row = MMQ_ITER_K / 8;
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
...
#pragma unroll
        for (int part = 0; part < 2; ++part) {
            ...
            const int block = word / words_per_block;
            const int word_in_block = word % words_per_block;
            const block_rocmfp2 * bxi = (const block_rocmfp2 *) x + kbx0 + i*stride + block;
#if defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_ROCMFPX + word] =
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + word] =
#endif
                rocmfpx_pack4_fp2_vec_cuda(bxi->qs, 4*word_in_block);
        }

        if (txi < MMQ_TILE_NE_K/2) {
            const int half = txi;
            const int block = half / 2;
            const block_rocmfp2 * bxi = (const block_rocmfp2 *) x + kbx0 + i*stride + block;
#if defined(AMD_WMMA_AVAILABLE)
            x_df[i*MMQ_MMA_TILE_X_K_ROCMFPX + half] = rocmfpx_ue4m3_to_fp32_finite(bxi->e[half & 1]);
#else
            x_df[i*(MMQ_TILE_NE_K/2) + i/2 + half] = rocmfpx_ue4m3_to_fp32_finite(bxi->e[half & 1]);
#endif
        }
    }
}

template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfp3(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    constexpr int words_per_block = QK_ROCMFP3 / 4;
    constexpr int threads_per_row = MMQ_ITER_K / 8;
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
...
#pragma unroll
        for (int part = 0; part < 2; ++part) {
            ...
            const int block = word / words_per_block;
            const int word_in_block = word % words_per_block;
            const block_rocmfp3 * bxi = (const block_rocmfp3 *) x + kbx0 + i*stride + block;
#if defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_ROCMFPX + word] =
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + word] =
#endif
                rocmfpx_pack4_fp3_vec_cuda(bxi->qs, 4*word_in_block);
        }
...
template <int mmq_x, int mmq_y>
static __device__ __forceinline__ void vec_dot_rocmfpx_q8_1_dp4a(
    const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q2_0_ROCMFP2, mmq_y);
...
#pragma unroll
    for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += VDR_ROCMFP2_Q8_1_MMQ) {
        const int k0 = k00 + k01;
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                ...
                int dot = 0;
#pragma unroll
                for (int l = 0; l < VDR_ROCMFP2_Q8_1_MMQ; ++l) {
                    dot = ggml_cuda_dp4a(x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + l],
                                         y_qs[j*MMQ_TILE_Y_K + (k01 + l) % MMQ_TILE_NE_K], dot);
                }
                const float dA = x_df[i*(MMQ_TILE_NE_K/2) + i/2 + k0/VDR_ROCMFP2_Q8_1_MMQ];
                sum[j0/nwarps*mmq_y/warp_size + i0/warp_size] += (float) dot * dA * dB;
            }
        }
    }
}
...
template <int mmq_x, int mmq_y>
static __device__ __forceinline__ void vec_dot_rocmfpx_q8_1_mma(
    const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
#if defined(AMD_WMMA_AVAILABLE)
...
#pragma unroll
        for (int n = 0; n < ntx; ++n) {
            load_generic(A[n], x_qs + (i0 + n*tile_A::I)*MMQ_MMA_TILE_X_K_ROCMFPX + k0,
                         MMQ_MMA_TILE_X_K_ROCMFPX);
        }
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    ...
                    const float dA = x_df[i*MMQ_MMA_TILE_X_K_ROCMFPX + k0/4];
...
#else
    vec_dot_rocmfpx_q8_1_dp4a<mmq_x, mmq_y>(x, y, sum, k00);
#endif
}
...
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q2_0_ROCMFP2> {
    ...
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_rocmfp2<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_rocmfpx_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_rocmfpx_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q3_0_ROCMFPX> {
    static constexpr int              vdr          = VDR_ROCMFP3_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_rocmfp3<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_rocmfpx_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_rocmfpx_q8_1_dp4a<mmq_x, mmq_y>;
};
...
#elif defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    constexpr bool rocmfpx_dp4a_layout = type == GGML_TYPE_Q2_0_ROCMFP2 || type == GGML_TYPE_Q3_0_ROCMFPX;
    constexpr vec_dot_mmq_t vec_dot = rocmfpx_dp4a_layout ?
        mmq_type_traits<mmq_x, mmq_y, need_check, type>::vec_dot_dp4a :
        mmq_type_traits<mmq_x, mmq_y, need_check, type>::vec_dot_mma;
    constexpr mmq_write_back_t write_back = rocmfpx_dp4a_layout ?
...
static size_t mmq_get_nbytes_shared(const int mmq_x, const int mmq_y, const int cc, const int warp_size, const int nwarps) {
    ...
    constexpr bool rocmfpx_type = type == GGML_TYPE_Q2_0_ROCMFP2 || type == GGML_TYPE_Q3_0_ROCMFPX;
    const bool use_mma_layout = rocmfpx_type ? amd_wmma_available(cc) :
