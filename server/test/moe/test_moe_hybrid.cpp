...
TEST_CASE(ServerUnitFixture, test_moe_hybrid_expert_compute_batch_default) {
    const luce_test::ScopedEnvVar batch("DFLASH_MOE_EXPERT_COMPUTE_BATCH", nullptr);
    const luce_test::ScopedEnvVar batch_max("DFLASH_MOE_EXPERT_COMPUTE_BATCH_MAX", nullptr);
...
TEST_CASE(ServerUnitFixture, test_moe_hybrid_expert_compute_ipc_mode_batch_limit) {
    {
        const luce_test::ScopedEnvVar mode("DFLASH_MOE_EXPERT_COMPUTE_IPC_MODE", nullptr);
        const luce_test::ScopedEnvVar cap("DFLASH_MOE_EXPERT_COMPUTE_IPC_BATCH_CAPACITY", nullptr);
        TEST_ASSERT(moe_hybrid_expert_compute_ipc_batch_limit(2048) == 1024);
    }

    // The capacity override stays set while the mode varies, matching the
    // pre-refactor sequence this case replaced.
    const luce_test::ScopedEnvVar cap("DFLASH_MOE_EXPERT_COMPUTE_IPC_BATCH_CAPACITY", "512");
    {
        const luce_test::ScopedEnvVar mode("DFLASH_MOE_EXPERT_COMPUTE_IPC_MODE", "auto");
        TEST_ASSERT(moe_hybrid_expert_compute_ipc_batch_limit(2048) == 512);
    }
    {
        const luce_test::ScopedEnvVar mode("DFLASH_MOE_EXPERT_COMPUTE_IPC_MODE", "batched");
        TEST_ASSERT(moe_hybrid_expert_compute_ipc_batch_limit(2048) == 512);
    }
    {
        const luce_test::ScopedEnvVar mode("DFLASH_MOE_EXPERT_COMPUTE_IPC_MODE", "stream");
...
TEST_CASE(ServerUnitFixture, test_moe_hybrid_prefill_hot_sub_batch_limit) {
    {
        const luce_test::ScopedEnvVar sub("DFLASH_MOE_PREFILL_HOT_SUB_BATCH", nullptr);
        TEST_ASSERT(moe_hybrid_prefill_hot_sub_batch_limit() == 4);
    }
    {
        const luce_test::ScopedEnvVar sub("DFLASH_MOE_PREFILL_HOT_SUB_BATCH", "0");
        TEST_ASSERT(moe_hybrid_prefill_hot_sub_batch_limit() == 4);
    }
    {
        const luce_test::ScopedEnvVar sub("DFLASH_MOE_PREFILL_HOT_SUB_BATCH", "3");
        TEST_ASSERT(moe_hybrid_prefill_hot_sub_batch_limit() == 3);
    }
    {
        const luce_test::ScopedEnvVar sub("DFLASH_MOE_PREFILL_HOT_SUB_BATCH", "8");
        TEST_ASSERT(moe_hybrid_prefill_hot_sub_batch_limit() == 4);
    }
}
...
TEST_CASE(ServerUnitFixture, test_moe_hybrid_canonical_rocmfp2_q2_is_tokenwise) {
    ...
    const luce_test::ScopedEnvVar moe_mmVQ("DFLASH_MOE_TP_GROUPED_MMVQ", nullptr);
    const luce_test::ScopedEnvVar ds4_mmVQ("DFLASH_DS4_TP_GROUPED_MMVQ", nullptr);
