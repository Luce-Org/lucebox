// gfx1151 paged dense dispatch regression test.
//
// Monolithic paged DeepSeek4 packs up to sixteen rows through plain mul_mat.
// Two dispatch policies preserve single-column arithmetic:
//
...
//      accumulates in F16.
//   2. ROCmFP4 dense projections through sixteen columns use weight-reuse
//      MMVQ instead of the MMQ tile.
//
...
bool test_rocmfp4_packed_columns(ggml_backend_t backend) {
    ...
    for (const Shape & shape : shapes) {
        ...
        std::vector<float> input((size_t) shape.k * 16);
        for (float & v : input) {
            v = lcg_uniform() * 3.0f;
        }
