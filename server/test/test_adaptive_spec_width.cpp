...
TEST_CASE(AdaptiveSpecWidthFixture, ling_dspark_reaches_all_eight_candidates) {
    AdaptiveSpecWidth width(9, 2, true, 4.0f);
    CHECK(width.next_width() == 5);
    for (int expected = 6; expected <= 9; ++expected) {
        const int offered = width.next_width();
        width.observe(offered, offered);
        CHECK(width.next_width() == expected);
    }
    CHECK(width.next_width() == 9);
}

TEST_CASE(AdaptiveSpecWidthFixture, hysteresis_requires_sustained_full_acceptance) {
    AdaptiveSpecWidth width(
        9, 2, true, 4.0f, 0.25f, 1.0f,
        /*full_accepts_per_probe=*/2);
    CHECK(width.next_width() == 5);
    width.observe(5, 5);
    CHECK(width.next_width() == 5);
    width.observe(5, 5);
...
TEST_CASE(AdaptiveSpecWidthFixture, parallel_probe_jumps_after_stable_baseline) {
    AdaptiveSpecWidth width(
        9, 2, true, 4.0f, 0.25f, 4.0f,
        /*full_accepts_per_probe=*/2);
    CHECK(width.next_width() == 5);
    width.observe(5, 5);
    CHECK(width.next_width() == 5);
...
TEST_CASE(AdaptiveSpecWidthFixture, wide_probe_does_not_overshoot_recovery) {
    AdaptiveSpecWidth width(
        9, 2, true, 4.0f, 0.25f, 4.0f,
        /*full_accepts_per_probe=*/2);
    width.observe(1, 5);
