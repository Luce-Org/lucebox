...
class DaemonResetMergeResolutionTest(unittest.TestCase):
    def test_daemon_reset_reuses_cache_and_frees_target_and_draft_graphs(self):
        ...
        if match is None:
            self.fail("daemon reset block not found")

        body = match.group("body")
        self.assertIn("step_graph_free(sg);", body)
        self.assertIn("step_graph_free(draft_sg);", body)
        self.assertIn("reset_target_cache(cache);", body)
        self.assertNotIn("step_graph_destroy(sg);", body)
