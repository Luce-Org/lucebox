...
// Deletes a temp directory tree on scope exit so a thrown TEST_ASSERT cannot
// leak fixture files into later cases sharing this test process.
namespace {
...
TEST_CASE(ServerUnitFixture, test_model_card_env_override_beats_cwd) {
    ...
    const ScopedTempDir root(fs::temp_directory_path() / "dflash-mc-env-test");
    const auto envdir = root.path / "explicit";
    fs::remove_all(root.path);
    ...
    {
        const luce_test::ScopedEnvVar env(
            "DFLASH_MODEL_CARDS_DIR", envdir.string().c_str());
        const auto card = dflash::common::resolve_model_card(
            "", "env-probe-model", "deepseek4", "");

        // Resolved from the env dir, not the deepseek4 family fallback
        // (which gives 32768).
