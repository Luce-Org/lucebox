...
TEST_CASE(ServerUnitFixture, test_prefix_cache_reserves_disk_staging_slot) {
    ...
    // The HTTP server owns backend slot kMaxSlots-1 for disk staging, so the
    // cache clamps its capacity one slot short.
    ...
    // Fill every clamped slot (plus overflow, which takes the eviction path)
    // through the real reservation API; the staging slot is never handed out.
    std::vector<bool> seen(PrefixCache::MAX_SLOTS, false);
    for (int i = 0; i < PrefixCache::MAX_CACHE_SLOTS + 8; ++i) {
        const std::vector<int32_t> prompt = {1, i, 2, 3};
        auto reservation = cache.reserve_inline_snap(
            prompt, /*restored_prefix_len=*/0, /*prefer_tools_boundary=*/false,
            /*forced_cut=*/2);
        TEST_ASSERT(reservation.active());
        TEST_ASSERT(reservation.slot() >= 0 &&
                    reservation.slot() < PrefixCache::MAX_CACHE_SLOTS);
        seen[(size_t)reservation.slot()] = true;
        TEST_ASSERT(reservation.commit(prompt));
    }
    for (int slot = 0; slot < PrefixCache::MAX_CACHE_SLOTS; ++slot) {
        TEST_ASSERT(seen[(size_t)slot]);
    }
