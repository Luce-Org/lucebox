...
Compares cold (--cache-ram=0) vs warm prefix-only cache
...
def run_config(label: str, port: int, prefix_ram_mib: int, user_turns: list[str],
    ...
    cache_args = ["--cache-ram", "0"]
    if prefix_ram_mib > 0:
        cache_args = [
            "--cache-ram", f"{prefix_ram_mib}MiB",
            "--cache-prefill-ram", "0",
        ]
    ...
    cache_label = "cache-ram=0" if prefix_ram_mib == 0 else (
        f"cache-ram={prefix_ram_mib}MiB cache-prefill-ram=0")
    print(f"\n--- {label} ({cache_label}) ---", flush=True)
