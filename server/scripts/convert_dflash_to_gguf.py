...
def load_arch(safetensors: Path, header: dict) -> dict:
    ...
    if cfg_path.exists():
        ...
        # Lives under dflash_config in newer repos. Used by the target loader
        # to set capture_layer_ids matching the drafter's training.
        tli = pick_dflash("target_layer_ids")
        if tli:
            a["target_layer_ids"] = list(tli)
...
def main():
    ...
    # Target layer IDs: exact target layers to capture (enables the target
    # loader to match the drafter's trained capture positions exactly,
    # instead of the evenly-spaced heuristic). Read by draft_gguf_loader.cpp
    # and the backend's early-read sync.
    if a.get("target_layer_ids"):
        writer.add_array(f"{ARCH}.dflash.target_layer_ids",
                         [int(x) for x in a["target_layer_ids"]])
