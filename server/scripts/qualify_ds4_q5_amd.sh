...
ATTENTION_TP_RATIO="${ATTENTION_TP_RATIO:--1}"
ATTENTION_TP_VALUES="${ATTENTION_TP_VALUES:-0}"
ATTENTION_TP_DIRECT_KV="${ATTENTION_TP_DIRECT_KV:-0}"
ATTENTION_TP_FLASH="${ATTENTION_TP_FLASH:-0}"
ATTENTION_TP_OUTPUT_B="${ATTENTION_TP_OUTPUT_B:-0}"
ATTENTION_TP_PROJECTION_ONLY="${ATTENTION_TP_PROJECTION_ONLY:-0}"
ATTENTION_TP_CORE_ONLY="${ATTENTION_TP_CORE_ONLY:-0}"
ATTENTION_TP_DEBUG_AB="${ATTENTION_TP_DEBUG_AB:-0}"
SCHED_PROFILE="${SCHED_PROFILE:-0}"
DST_STREAM_PEER_COPIES="${DST_STREAM_PEER_COPIES:-0}"
ATTENTION_TP_PACKED_STAGE="${ATTENTION_TP_PACKED_STAGE:-0}"
SINGLE_COPY_EVENT_FENCES="${SINGLE_COPY_EVENT_FENCES:-0}"
DRAFT_GPU="${DRAFT_GPU:-0}"
DRAFT_SEPARATE_STREAM="${DRAFT_SEPARATE_STREAM:-0}"
DRAFT_LOW_PRIORITY="${DRAFT_LOW_PRIORITY:-0}"
DRAFT_OVERLAP_PROBE="${DRAFT_OVERLAP_PROBE:-0}"
DRAFT_OVERLAP_REUSE_CONTEXT="${DRAFT_OVERLAP_REUSE_CONTEXT:-0}"
DRAFT_DEVICE_CHAIN="${DRAFT_DEVICE_CHAIN:-0}"
MMID_GROUP_REUSE="${MMID_GROUP_REUSE:-0}"
DUPLICATE_HOT_ON_COLD="${DUPLICATE_HOT_ON_COLD:-1}"
FULL_COLD_PARALLEL="${FULL_COLD_PARALLEL:-1}"
...
for toggle in "$DRAFT_SEPARATE_STREAM" "$DRAFT_LOW_PRIORITY" \
              "$DRAFT_OVERLAP_PROBE" "$DRAFT_OVERLAP_REUSE_CONTEXT" \
              "$DRAFT_DEVICE_CHAIN" "$MMID_GROUP_REUSE" \
              "$DUPLICATE_HOT_ON_COLD" "$FULL_COLD_PARALLEL"; do
    case "$toggle" in
        0|1) ;;
        *) echo "draft stream/probe and MMID reuse controls must be 0 or 1" >&2; exit 2 ;;
    esac
done
if ! [[ "$DRAFT_GPU" =~ ^[0-9]+$ ]]; then
    echo "DRAFT_GPU must be a non-negative integer" >&2
    exit 2
fi
...
if [[ ! "$ATTENTION_TP_GROUPS" =~ ^[0-9]+$ ]] || ((ATTENTION_TP_GROUPS > 7)); then
    echo "ATTENTION_TP_GROUPS must be an integer from 0 through 7" >&2
    exit 2
fi
if [[ ! "$ATTENTION_TP_RATIO" =~ ^(-1|[0-9]+)$ ]]; then
    echo "ATTENTION_TP_RATIO must be -1 or a non-negative integer" >&2
    exit 2
fi
case "$ATTENTION_TP_DEBUG_AB" in
    0|1) ;;
    *) echo "ATTENTION_TP_DEBUG_AB must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_VALUES" in
    0|1) ;;
    *) echo "ATTENTION_TP_VALUES must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_DIRECT_KV" in
    0|1) ;;
    *) echo "ATTENTION_TP_DIRECT_KV must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_FLASH" in
    0|1) ;;
    *) echo "ATTENTION_TP_FLASH must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_OUTPUT_B" in
    0|1) ;;
    *) echo "ATTENTION_TP_OUTPUT_B must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_PROJECTION_ONLY" in
    0|1) ;;
    *) echo "ATTENTION_TP_PROJECTION_ONLY must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_CORE_ONLY" in
    0|1) ;;
    *) echo "ATTENTION_TP_CORE_ONLY must be 0 or 1" >&2; exit 2 ;;
esac
if ((ATTENTION_TP_PROJECTION_ONLY + ATTENTION_TP_CORE_ONLY > 1)); then
    echo "ATTENTION_TP_PROJECTION_ONLY and ATTENTION_TP_CORE_ONLY are mutually exclusive" >&2
    exit 2
fi
case "$SCHED_PROFILE" in
    0|1) ;;
    *) echo "SCHED_PROFILE must be 0 or 1" >&2; exit 2 ;;
esac
case "$DST_STREAM_PEER_COPIES" in
    0|1) ;;
    *) echo "DST_STREAM_PEER_COPIES must be 0 or 1" >&2; exit 2 ;;
esac
case "$ATTENTION_TP_PACKED_STAGE" in
    0|1) ;;
    *) echo "ATTENTION_TP_PACKED_STAGE must be 0 or 1" >&2; exit 2 ;;
esac
case "$SINGLE_COPY_EVENT_FENCES" in
    0|1) ;;
    *) echo "SINGLE_COPY_EVENT_FENCES must be 0 or 1" >&2; exit 2 ;;
esac
...
server_env=(
    ...
    "DFLASH_DS4_ATTENTION_TP_GROUPS=$ATTENTION_TP_GROUPS"
    "DFLASH_DS4_ATTENTION_TP_RATIO=$ATTENTION_TP_RATIO"
    "DFLASH_DS4_ATTENTION_TP_VALUES=$ATTENTION_TP_VALUES"
    "DFLASH_DS4_ATTENTION_TP_DIRECT_KV=$ATTENTION_TP_DIRECT_KV"
    "DFLASH_DS4_ATTENTION_TP_FLASH=$ATTENTION_TP_FLASH"
    "DFLASH_DS4_ATTENTION_TP_OUTPUT_B=$ATTENTION_TP_OUTPUT_B"
    "DFLASH_DS4_ATTENTION_TP_PROJECTION_ONLY=$ATTENTION_TP_PROJECTION_ONLY"
    "DFLASH_DS4_ATTENTION_TP_CORE_ONLY=$ATTENTION_TP_CORE_ONLY"
    "DFLASH_DS4_ATTENTION_TP_DEBUG_AB=$ATTENTION_TP_DEBUG_AB"
    "DFLASH_DS4_ATTENTION_TP_DST_STREAM_STAGE=$DST_STREAM_PEER_COPIES"
    "DFLASH_DS4_ATTENTION_TP_PACKED_STAGE=$ATTENTION_TP_PACKED_STAGE"
    "DFLASH_DS4_TP_SCHED_TRACE=$ATTENTION_TP_DEBUG_AB"
    "GGML_SCHED_PROFILE=$SCHED_PROFILE"
    "GGML_SCHED_PROFILE_MIN_SPLITS=100"
    "GGML_CUDA_BATCH_PEER_COPY_TRACE=0"
    ...
    "GGML_CUDA_DST_STREAM_PEER_COPIES=$DST_STREAM_PEER_COPIES"
    "GGML_SCHED_SINGLE_COPY_EVENT_FENCES=$SINGLE_COPY_EVENT_FENCES"
    "DFLASH_MOE_DUPLICATE_HOT_ON_COLD=$DUPLICATE_HOT_ON_COLD"
    "DFLASH_DS4_HYBRID_PREFILL_GPU_HC=1"
    "DFLASH_DS4_HYBRID_PREFILL_EAGER=1"
    "DFLASH_MOE_FULL_COLD_PARALLEL=$FULL_COLD_PARALLEL"
    ...
    "DFLASH_DS4_DRAFT_GPU=$DRAFT_GPU"
    "DFLASH_DS4_DRAFT_SEPARATE_STREAM=$DRAFT_SEPARATE_STREAM"
    "DFLASH_DS4_DRAFT_LOW_PRIORITY=$DRAFT_LOW_PRIORITY"
    "DFLASH_DS4_DRAFT_OVERLAP_PROBE=$DRAFT_OVERLAP_PROBE"
    "DFLASH_DS4_DRAFT_OVERLAP_REUSE_CONTEXT=$DRAFT_OVERLAP_REUSE_CONTEXT"
    "DFLASH_DS4_DRAFT_DEVICE_CHAIN=$DRAFT_DEVICE_CHAIN"
    "DFLASH_CUDA_MMVQ_MOE_GROUP_REUSE=$MMID_GROUP_REUSE"
...

{
    ...
    echo "attention_tp_ratio=$ATTENTION_TP_RATIO"
    echo "attention_tp_values=$ATTENTION_TP_VALUES"
    echo "attention_tp_direct_kv=$ATTENTION_TP_DIRECT_KV"
    echo "attention_tp_flash=$ATTENTION_TP_FLASH"
    echo "attention_tp_output_b=$ATTENTION_TP_OUTPUT_B"
    echo "attention_tp_projection_only=$ATTENTION_TP_PROJECTION_ONLY"
    echo "attention_tp_core_only=$ATTENTION_TP_CORE_ONLY"
    echo "attention_tp_debug_ab=$ATTENTION_TP_DEBUG_AB"
    echo "dst_stream_peer_copies=$DST_STREAM_PEER_COPIES"
    echo "attention_tp_packed_stage=$ATTENTION_TP_PACKED_STAGE"
    echo "single_copy_event_fences=$SINGLE_COPY_EVENT_FENCES"
    echo "sched_profile=$SCHED_PROFILE"
    echo "draft_gpu=$DRAFT_GPU"
    echo "draft_separate_stream=$DRAFT_SEPARATE_STREAM"
    echo "draft_low_priority=$DRAFT_LOW_PRIORITY"
    echo "draft_overlap_probe=$DRAFT_OVERLAP_PROBE"
    echo "draft_overlap_reuse_context=$DRAFT_OVERLAP_REUSE_CONTEXT"
    echo "draft_device_chain=$DRAFT_DEVICE_CHAIN"
    echo "mmid_group_reuse=$MMID_GROUP_REUSE"
    echo "duplicate_hot_on_cold=$DUPLICATE_HOT_ON_COLD"
    echo "full_cold_parallel=$FULL_COLD_PARALLEL"
