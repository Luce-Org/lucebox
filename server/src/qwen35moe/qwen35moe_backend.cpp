...
GenerateResult Qwen35MoeBackend::generate_impl(const GenerateRequest & req,
    ...
    if (req.n_gen > 0) {
        ...
        if (can_hybrid_spec) {
            ...
            cleanup_graphs();
            if (!do_hybrid_spec_decode(committed, req.n_gen, result.tokens,
                                       out_io, result.accept_rate)) {
        ...
        if (!io.cancelled && !hit_eos && fallback_steps > 0 &&
            ...
            if (out_accept_rate < min_accept) {
                ...
                std::fprintf(stderr,
                    ...
                    out_accept_rate, min_accept, n_draft_steps, ar_n_gen);
                step_graph_destroy(draft_sg);
                target_cache().last_tok = last_tok;
...
bool Qwen35MoeBackend::load_dynamic_placement(const char * hotness_path,
    ...
    // On UMA / HIP backends the memory query can report free > total while the
    // model still fits. Clamp the subtraction to keep the budget arithmetic
    // stable instead of underflowing.
    const uint64_t core_bytes = gpu_total > gpu_free ? gpu_total - gpu_free : 0;
