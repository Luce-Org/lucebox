...
Qwen35MoeLayerSplitAdapter::Qwen35MoeLayerSplitAdapter(
        const Qwen35MoeLayerSplitAdapterConfig & cfg)
    : cfg_(cfg) {}
...
void Qwen35MoeLayerSplitAdapter::kvflash_read_config() {
...
bool Qwen35MoeLayerSplitAdapter::kvflash_attach() {
...
bool Qwen35MoeLayerSplitAdapter::kvflash_sync_identity(int committed) {
    if (!kvflash_active()) return true;
...
void Qwen35MoeLayerSplitAdapter::kvflash_sync_history(
...
void Qwen35MoeLayerSplitAdapter::kvflash_maybe_reselect(int generated) {
...
bool Qwen35MoeLayerSplitAdapter::init_mixed_target_split() {
...
bool Qwen35MoeLayerSplitAdapter::load_draft() {
...
void Qwen35MoeLayerSplitAdapter::begin_request(const GenerateRequest & req) {
...
void Qwen35MoeLayerSplitAdapter::reset_request_state() {
...
int Qwen35MoeLayerSplitAdapter::prefill_chunk_tokens() const {
    if (kvflash_active()) {
...
bool Qwen35MoeLayerSplitAdapter::prefill(const std::vector<int32_t> & prompt,
...
bool Qwen35MoeLayerSplitAdapter::snapshot_slot_valid(int slot) const {
    return slot >= 0 && slot < PREFIX_SLOTS &&
           prefix_snapshots_.size() == (size_t)PREFIX_SLOTS &&
           !shards_.empty();
}

bool Qwen35MoeLayerSplitAdapter::snapshot_save(int slot) {
...
void Qwen35MoeLayerSplitAdapter::snapshot_free(int slot) {
...
bool Qwen35MoeLayerSplitAdapter::snapshot_used(int slot) const {
    if (!snapshot_slot_valid(slot)) return false;
...
int Qwen35MoeLayerSplitAdapter::snapshot_cur_pos(int slot) const {
    if (!snapshot_used(slot)) return 0;
    return prefix_snapshots_[(size_t)slot].front().cur_pos;
}

bool Qwen35MoeLayerSplitAdapter::snapshot_restore(int slot) {
...
bool Qwen35MoeLayerSplitAdapter::rebuild_disk_snapshot(int slot) {
    if (!snapshot_slot_valid(slot)) return false;
    if (disk_snapshot_contexts_.size() != (size_t)PREFIX_SLOTS ||
...
ModelBackend::SnapshotRef Qwen35MoeLayerSplitAdapter::snapshot_ref(int slot) const {
...
bool Qwen35MoeLayerSplitAdapter::snapshot_adopt(int slot,
                                             ggml_context * ctx,
...
bool Qwen35MoeLayerSplitAdapter::snapshot_draft_features(int slot) {
    if (!cfg_.run_dflash || !cfg_.draft_path) {
        free_draft_feature_snapshot(slot);
        return true;
    }
...
void Qwen35MoeLayerSplitAdapter::free_draft_feature_snapshot(int slot) {
...
bool Qwen35MoeLayerSplitAdapter::restore_draft_features(int slot) {
...
int Qwen35MoeLayerSplitAdapter::current_last_token() const {
    if (shards_.empty()) return -1;
    return shards_.front().cache.last_tok;
}

bool Qwen35MoeLayerSplitAdapter::decode_ar(
...
bool Qwen35MoeLayerSplitAdapter::can_dflash_decode() const {
    return cfg_.run_dflash && cfg_.draft_path && !sampler_.needs_logit_processing();
}

bool Qwen35MoeLayerSplitAdapter::decode_dflash(
...
const char * Qwen35MoeLayerSplitAdapter::default_compress_drafter_path() const {
    return "/opt/lucebox/models/drafter/Qwen3-0.6B-BF16.gguf";
}

ModelBackend::CompressResult
Qwen35MoeLayerSplitAdapter::compress(const ModelBackend::CompressRequest & req) {
...
void Qwen35MoeLayerSplitAdapter::free_drafter() {
    remote_draft_.close();
    if (pflash_drafter_loaded_) {
...
DFlashTarget * Qwen35MoeLayerSplitAdapter::dflash_target() {
    if (!dflash_target_) {
...
void Qwen35MoeLayerSplitAdapter::shutdown() {
