...
bool Qwen35Backend::init() {
    ...
    // Early-read drafter capture config so the target graph is built with
    // the correct fc_in = n_capture_layers * n_embd. Newer 8-tap drafters
    // need 8 captured layers; without this sync the default 5 would cause
    // a runtime shape mismatch on the drafter's fc projection.
    if (cfg_.draft_path) {
    ...
    if (cfg_.draft_path && use_remote_draft) {
        ...
        // Propagate the draft's mask_token_id to the target weights. The
        // noise-generation code (which fills MASK tokens for block diffusion)
        // reads target_weights().mask_token_id, so it must match the drafter's
        // trained mask token (248077 for Qwen3.6, vs the old 248070 default).
        if (dw_.mask_token_id != w_.mask_token_id) {
            std::printf("[draft]  mask_token_id %d -> %d (from draft GGUF)\n",
                        w_.mask_token_id, dw_.mask_token_id);
            w_.mask_token_id = dw_.mask_token_id;
        }
    ...
    if (std::getenv("DFLASH_KVFLASH") && !kvflash_qk_policy_) {
        ...
        // Parse DFLASH_KVFLASH_PIN_SPANS="lo:hi,lo:hi" token ranges.
        if (const char * ps = std::getenv("DFLASH_KVFLASH_PIN_SPANS")) {
            const char * p = ps;
            while (*p) {
                long lo = 0, hi = 0;
                char * end = nullptr;
                lo = std::strtol(p, &end, 10);
                if (end == p || *end != ':') break;
                p = end + 1;
                hi = std::strtol(p, &end, 10);
                if (end == p) break;
                if (lo >= 0 && hi >= lo) {
                    kvflash_pin_spans_.push_back({(int)lo, (int)hi});
                }
                p = end;
                if (*p == ',') p++;
            }
            if (!kvflash_pin_spans_.empty()) {
                std::fprintf(stderr, "[kvflash] pin spans: %d range(s) configured\n",
                             (int)kvflash_pin_spans_.size());
            }
        }
    }
    ...
    // Subclass gate (e.g. MoE all-hot fit check): may zero kvflash_tokens_ to
    // disable the pool when it is not needed.  Called before create_target_cache
    // so the cache is allocated at the right size.
    // Stash the budget so the gate can reuse the bytes_per_token/free_bytes without
    // re-querying GPU memory.
    kvf_budget_ = kvf_budget;
...
static const std::vector<uint8_t> s_empty_blob;
const std::vector<uint8_t> & Qwen35Backend::snapshot_kvflash_blob(int slot) const {
    if (slot < 0 || slot >= PREFIX_SLOTS) return s_empty_blob;
...
bool Qwen35Backend::snapshot_adopt(int slot, ggml_context * ctx,
    ...
    // Pooled snapshots have no attn_k/v (KV lives in kvflash_blob); skip that check.
    if (!snap.is_pooled) {
...
void Qwen35Backend::apply_kvflash_pins() {
    if (kvflash_pin_spans_.empty()) return;
    for (const auto & span : kvflash_pin_spans_) {
        kvflash_pager_.pin_range((int64_t)span.first, (int64_t)span.second);
    }
}
...
void Qwen35Backend::kvflash_maybe_reselect(int generated) {
    ...
    // DFLASH_KVFLASH_TAU_EXACT=1 bypasses adaptive scaling so the configured
    // value is used exactly (useful for testing / short-decode validation).
    static const bool tau_exact = [] {
        const char * e = std::getenv("DFLASH_KVFLASH_TAU_EXACT");
        return e && std::strcmp(e, "0") != 0;
    }();
    const int tau = tau_exact
        ? kvflash_tau_
        : std::max<int>(kvflash_tau_, (int)(kvflash_history_.size() / 45));
...
bool Qwen35Backend::do_spec_decode(int committed, int n_gen,
    ...
    // ── Realized-speedup gate config (read once) ─────────────────────────
    // Replaces the old p1-entropy gate with a self-calibrating timing gate:
    ...
    while (n_generated < n_gen) {
        ...
        if (use_remote_draft) {
            ...
            if (!use_mirror_view &&
                !copy_feature_ring_range_to_tensor(feature_mirror_, draft_sg_.target_hidden_cat,
            ...
            ggml_backend_tensor_set(draft_sg_.positions, pos_q.data(), 0,
                                    sizeof(int32_t) * pos_q.size());
            ggml_backend_tensor_set(draft_sg_.positions_k, pos_k.data(), 0,
            ...
            ggml_backend_tensor_get(draft_sg_.hidden_states, local_hidden.data(), 0,
        ...
        if (cfg_.ddtree_mode && target->supports_tree_verify() &&
            ...
            if (accepted_n <= 0) { step_graph_destroy(draft_sg_); break; }
            ...
            if (accepted_emitted <= 0) { step_graph_destroy(draft_sg_); break; }
        ...
        //    Env-tunable (DFLASH_FAST_ROLLBACK_MIN) for A/B; default 5.
        static const int kFastRollbackThreshold = []() {
            const char * e = std::getenv("DFLASH_FAST_ROLLBACK_MIN");
            return e ? std::atoi(e) : 5;
        }();
