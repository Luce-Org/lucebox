...
int Qwen35SeqEngine::prefill_chunk_size(int slot, int max_tokens) const {
    if (!slots_.is_prefilling(slot)) return 0;
    const Qwen35Slot & seq = slots_.slot(slot);
    int chunk = std::min(max_tokens, seq.prompt_len - seq.cur_pos);
    const PrefixCaptureTicket capture = seq.pending_capture;
    if (capture.valid() && seq.cur_pos < capture.checkpoint.tokens) {
        // Recurrent state is checkpoint-consistent only after compute.
        chunk = std::min(chunk, capture.checkpoint.tokens - seq.cur_pos);
    }
    return std::max(0, chunk);
}
...
Qwen35SeqEngine::PrefillStage Qwen35SeqEngine::stage_prefill_chunk(
    ...
    stage.chunk = prefill_chunk_size(slot, max_tokens);
...
SeqEngine::StepResult Qwen35SeqEngine::step_chain_spec(
    ...
    for (size_t i = 0; i < prefills.size(); ++i) {
        ...
        const PrefixCaptureTicket capture =
            slots_.slot(output.slot).pending_capture;
        if (capture.valid() &&
            slots_.slot(output.slot).cur_pos == capture.checkpoint.tokens) {
            output.prefix_store = capture_prefix(output.slot, capture);
            slots_.slot(output.slot).pending_capture = {};
        }
