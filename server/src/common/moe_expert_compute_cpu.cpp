...
public:
    explicit CpuMoeExpertCompute(int n_ff_max, int n_threads = 0) : n_ff_max_(n_ff_max) {
        ...
        if (n_threads <= 0) {
            ...
            if (!env || !*env) env = std::getenv("DFLASH_COLD_THREADS");
        ...
        n_threads_ = 1;
#endif
        fprintf(stderr, "[moe-expert-compute] cpu threads=%d\n", n_threads_);
    ...
    bool compute(
        ...
        if (n_selected < 0 || n_embd <= 0 || n_ff <= 0 || !input || !ids ||
            !weights || !output) {
            return false;
        }
