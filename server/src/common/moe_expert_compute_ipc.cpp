...
struct MoeExpertIpcClientStats {
    ...
    void maybe_print(bool force) const {
        ...
        std::fprintf(stderr,
            ...
            " payload_mib=%.3f response_mib=%.3f"
            " pack_ms=%.3f send_ms=%.3f wait_ms=%.3f recv_ms=%.3f\n",
            calls, payload_bytes / 1048576.0,
            response_bytes / 1048576.0,
...
struct MoeExpertDaemonStats {
    ...
    void maybe_print(bool force, const char * scope = "") const {
        ...
        std::fprintf(stderr,
            ...
            " payload_mib=%.3f response_mib=%.3f builds=%" PRIu64
            " reuses=%" PRIu64
            " read_ms=%.3f unpack_ms=%.3f build_ms=%.3f set_ms=%.3f"
            " compute_ms=%.3f get_ms=%.3f write_ms=%.3f\n",
            scope, calls, payload_bytes / 1048576.0,
            response_bytes / 1048576.0, graph_builds, graph_reuses,
...
bool write_temp_placement_file(const MoeHybridPlacement & placement,
    ...
    return false;
#else
    char templ[] = "/tmp/lucebox_moe_target_placement_XXXXXX";
    int fd = ::mkstemp(templ);
    if (fd < 0) {
        if (err) *err = std::string("mkstemp failed: ") + std::strerror(errno);
        return false;
    ...
    ::close(fd);
    path_out = templ;
    if (!placement.save_json(path_out, arch_name ? arch_name : "moe_expert_target", err)) {
        ::unlink(path_out.c_str());
        path_out.clear();
        return false;
    }
...
bool write_named_placement_file(const MoeHybridPlacement & placement,
                                const std::string & path,
                                const char * arch_name,
                                std::string * err) {
    return placement.save_json(path, arch_name ? arch_name : "moe_expert_target", err);
}
...
size_t moe_expert_required_shared_bytes(int n_embd,
    ...
    if (!moe_expert_compute_checked_mul_size(input_elems, sizeof(float),
                                             response_bytes)) {
        return 0;
    }
    return std::max(total, response_bytes);
}
...
bool build_remote_moe_runtime_from_weights(
        ...
        const GgufTensorDataReader & expert_reader,
    ...
    const auto layer_file_data =
        make_layer_expert_file_data(expert_reader, weights.n_layer);
    std::fprintf(stderr,
...
bool load_remote_moe_runtime(const char * target_path,
    ...
    if (arch == "qwen35moe") {
        ...
        return build_remote_moe_runtime_from_weights(
            backend, placement, expert_reader, arch, weights,
            free_target_weights, out, err);
    }
    if (arch == "laguna") {
        ...
        return build_remote_moe_runtime_from_weights(
            backend, placement, expert_reader, arch, weights,
            free_laguna_target_weights, out, err);
    }
...
public:
    explicit MoeExpertComputeIpc(int n_ff_max, bool enable_fallback = true)
        : fallback_(enable_fallback ? make_cpu_moe_expert_compute(n_ff_max) : nullptr) {}
    ...
    bool start(const std::string & bin,
        ...
        launch.shared_payload_bytes = moe_expert_shared_bytes_from_env(
            moe_expert_required_shared_bytes(
                n_embd, n_expert_used, shared_batch_capacity, input_type_));
        ...
        const size_t max_input_bytes =
            moe_expert_ipc_input_row_size(input_type_, (size_t)n_embd_) *
            (size_t)std::max(1, moe_expert_compute_batch_limit_from_env());
        if (max_input_bytes > input_scratch_.capacity()) {
            input_scratch_.reserve(max_input_bytes);
        }
    ...
    bool start_for_placement(const std::string & bin,
                             const std::string & target_path,
                             int target_gpu,
                             ...
                             bool required) {
#if defined(_WIN32)
        (void)bin; (void)target_path; (void)target_gpu; (void)remote_placement;
        (void)n_embd; (void)n_expert_used; (void)work_dir; (void)required;
        return false;
#else
        std::string err;
        if (!write_temp_placement_file(remote_placement, "moe_remote_expert_target",
                                       placement_path_, &err)) {
            std::fprintf(stderr, "[moe-expert-compute-ipc] placement write failed: %s\n",
                         err.c_str());
            return false;
    ...
    bool compute_batch(const MoeExpertLayer & layer,
        ...
        const int daemon_batch_limit = moe_expert_compute_daemon_batch_limit_from_env();
        if (n_tokens > daemon_batch_limit) {
            for (int t0 = 0; t0 < n_tokens; t0 += daemon_batch_limit) {
                const int tc = std::min(daemon_batch_limit, n_tokens - t0);
                if (!compute_batch(
                        layer,
                        input + (size_t)t0 * (size_t)n_embd,
                        ids + (size_t)t0 * (size_t)n_selected,
                        weights + (size_t)t0 * (size_t)n_selected,
                        tc,
                        n_selected,
                        n_embd,
                        n_ff,
                        output + (size_t)t0 * (size_t)n_embd)) {
                    return false;
                }
            }
            return true;
    ...
    bool prepare_batch(const MoeExpertLayer & layer,
                       ...
                       int n_ff) override {
        (void)n_ff;
        if (n_tokens < 0 || n_selected < 0 || n_embd <= 0) return false;
        if (n_tokens == 0 || n_selected == 0) return true;
        if (!active_ || layer.layer_idx < 0 || n_embd != n_embd_) {
            return !required_ && fallback_ != nullptr;
        }
        if (n_selected > n_expert_used_ ||
            n_selected > (int)layer.cold_global_by_local.size()) {
            return !required_ && fallback_ != nullptr;
        }
        return prepare_remote_batch(layer.layer_idx, n_tokens, n_selected);
    }
    ...
    bool prepare_single(const MoeExpertLayer & layer,
                        ...
                        int n_ff) override {
        (void)n_ff;
        if (n_selected < 0 || n_embd <= 0) return false;
        ...
        return prepare_remote_single(layer.layer_idx, n_selected);
    }
...
private:
    bool compute_remote(int layer_idx,
        ...
        if (profile_) {
            ...
            stats_.response_bytes += output_bytes;
    ...
    bool prepare_remote_batch(int layer_idx, int n_tokens, int n_selected) {
#if defined(_WIN32)
        (void)layer_idx; (void)n_tokens; (void)n_selected;
        return false;
#else
        std::lock_guard<std::mutex> lock(command_mu_);
        FILE * cmd = process_.command_stream();
        const int stream_fd = process_.stream_fd();
        if (!cmd || stream_fd < 0 || layer_idx < 0 ||
            n_tokens <= 0 || n_selected <= 0 ||
            n_selected > n_expert_used_) {
            return false;
        }
        if (input_type_ == GGML_TYPE_F32) {
            std::fprintf(cmd, "prepare_batch_local %d %d %d\n",
                         layer_idx, n_tokens, n_selected);
        } else {
            std::fprintf(cmd, "prepare_batch_local_typed %d %d %d %d\n",
                         layer_idx, n_tokens, n_selected, (int)input_type_);
        }
        std::fflush(cmd);
        ...
        if (!read_exact_fd(stream_fd, &status, sizeof(status)) || status != 0) {
            std::fprintf(stderr,
                         "[moe-expert-compute-ipc] prepare_batch failed status=%d\n",
                         status);
            return false;
        }
        return true;
    ...
    bool prepare_remote_single(int layer_idx, int n_selected) {
#if defined(_WIN32)
        (void)layer_idx; (void)n_selected;
        return false;
#else
        std::lock_guard<std::mutex> lock(command_mu_);
        FILE * cmd = process_.command_stream();
        const int stream_fd = process_.stream_fd();
        if (!cmd || stream_fd < 0 || layer_idx < 0 ||
            n_selected <= 0 || n_selected > n_expert_used_) {
            return false;
        }
        // Decode-path compute currently consumes F32 hidden states, so the
        // single-token warmup must build the same F32 CachedFfnGraph.
        std::fprintf(cmd, "prepare_single_local %d %d\n",
                     layer_idx, n_selected);
        std::fflush(cmd);
        int32_t status = -1;
        if (!read_exact_fd(stream_fd, &status, sizeof(status)) || status != 0) {
            std::fprintf(stderr,
                         "[moe-expert-compute-ipc] prepare_single failed status=%d\n",
                         status);
            return false;
        }
        return true;
...
MoeExpertComputeIpcStartResult make_moe_expert_compute_ipc_for_placement(
    ...
    bool required) {
    MoeExpertComputeIpcStartResult result;
    auto compute = std::make_unique<MoeExpertComputeIpc>(
        n_ff_exp, /*enable_fallback=*/false);
    if (!compute->start_for_placement(bin, target_path, target_gpu, remote_placement,
                                      n_embd, n_expert_used, work_dir, required)) {
        return result;
    }
    result.started_remote = true;
...
int run_moe_expert_compute_ipc_daemon(const char * target_path,
    ...
    std::fprintf(stderr, "[moe-expert-compute-daemon] opening expert GGUF shards\n");
    if (!expert_reader.open(target_path,
                            /*build_merged_tensor_context=*/false, err) ||
        !expert_reader.open_mmaps(err)) {
        std::fprintf(stderr, "[moe-expert-compute-daemon] expert GGUF open failed: %s\n",
                     err.c_str());
        emit_status(stream_fd, -1);
        ggml_backend_free(backend);
        if (shared_payload && shared_payload != MAP_FAILED) {
    ...
    std::fprintf(stderr,
                 "[moe-expert-compute-daemon] expert GGUF shards ready count=%d\n",
                 expert_reader.shard_count());

    RemoteMoeRuntime runtime;
    if (!load_remote_moe_runtime(target_path, backend, placement,
                                 expert_reader, runtime, &err)) {
    ...
    input.reserve((size_t)runtime.n_embd * (size_t)reserve_batch_limit *
                  sizeof(float));
    output.reserve((size_t)runtime.n_embd * (size_t)reserve_batch_limit);
    batch_local_ids.reserve((size_t)reserve_batch_limit * (size_t)runtime.n_expert_used);
    payload_ids.reserve((size_t)reserve_batch_limit * (size_t)runtime.n_expert_used);
    payload_weights.reserve((size_t)reserve_batch_limit * (size_t)runtime.n_expert_used);
    auto record_graph_build = [&](MoeExpertDaemonStats & scope_stats,
                                  uint64_t build_us) {
        profile_stats.graph_builds++;
        profile_stats.graph_build_us += build_us;
        scope_stats.graph_builds++;
        scope_stats.graph_build_us += build_us;
    };
    auto record_graph_reuse = [&](MoeExpertDaemonStats & scope_stats) {
        profile_stats.graph_reuses++;
        scope_stats.graph_reuses++;
    };
    int warmup_builds = 0;
    std::fprintf(stderr,
    ...
    while (std::getline(std::cin, line)) {
        ...
        bool prepare_single_only = false;
        ggml_type input_type = GGML_TYPE_F32;
        ...
        if (cmd == "compute_pipe") {
            ...
            ids_are_local = true;
            input_type = (ggml_type)input_type_i;
        } else if (cmd == "prepare_batch_local") {
            iss >> layer_idx >> n_tokens >> n_selected;
            ids_are_local = true;
            prepare_only = true;
        } else if (cmd == "prepare_batch_local_typed") {
            int input_type_i = (int)GGML_TYPE_F32;
            iss >> layer_idx >> n_tokens >> n_selected >> input_type_i;
            ids_are_local = true;
            prepare_only = true;
            input_type = (ggml_type)input_type_i;
        } else if (cmd == "prepare_single_local") {
            iss >> layer_idx >> n_selected;
            n_tokens = 1;
            ids_are_local = true;
            prepare_only = true;
            prepare_single_only = true;
        } else {
            emit_status(stream_fd, -1);
            continue;
        }
        ...
        if (prepare_only) {
            if (prepare_single_only && input_type == GGML_TYPE_F32) {
                ...
                if (!graph.valid() || graph.n_hot != n_selected) {
                    ...
                    if (profile) {
                        record_graph_build(profile_stats_decode,
                                           moe_expert_compute_elapsed_us(build_t0, build_t1));
                    }
                } else if (profile) {
                    record_graph_reuse(profile_stats_decode);
                }
                emit_status(stream_fd, 0);
                continue;
            }
            ...
            if (!graph.valid() || graph.n_tokens != n_tokens ||
                ...
                if (profile) {
                    ...
                    record_graph_build(scope_stats,
                                       moe_expert_compute_elapsed_us(build_t0, build_t1));
                }
            } else if (profile) {
                MoeExpertDaemonStats & scope_stats =
                    n_tokens > 1 ? profile_stats_prefill : profile_stats_decode;
                record_graph_reuse(scope_stats);
            }
            emit_status(stream_fd, 0);
            continue;
        }
        ...
        if (n_tokens > 1 || input_type != GGML_TYPE_F32) {
            ...
            if (profile) {
                ...
                auto record_stats = [&](MoeExpertDaemonStats & stats) {
                    ...
                    stats.response_bytes += output_bytes;
        ...
        if (profile) {
            ...
            auto record_stats = [&](MoeExpertDaemonStats & stats) {
                stats.calls++;
                stats.payload_bytes += bytes;
                stats.response_bytes += input_bytes;
                stats.read_us += read_us;
                stats.unpack_us += unpack_us;
