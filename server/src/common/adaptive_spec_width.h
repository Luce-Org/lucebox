...
public:
    AdaptiveSpecWidth(int max_width, int min_width = 2, bool enabled = true,
        ...
        : max_width_(std::max(1, max_width)),
          ...
          full_accepts_per_probe_(std::max(1, full_accepts_per_probe)),
    ...
    void observe(int accepted_width, int offered_width) {
        ...
        if (accepted_candidates >= offered_candidates) {
            // Recover to the initial baseline immediately after a backoff.
            // Above that baseline, optional hysteresis prevents one lucky
            // block from opening a more expensive target shape.
            const int estimated_width = std::clamp(
                static_cast<int>(std::lround(accepted_candidates_ema_)) + 1,
                min_width_, max_width_);
            const int initial_width = std::clamp(
                static_cast<int>(
                    std::lround(initial_accepted_candidates_)) + 1,
                min_width_, max_width_);
            ++full_accept_streak_;
            if (estimated_width < initial_width ||
                full_accept_streak_ >= full_accepts_per_probe_) {
                // Censored lower bound: do not average it downward; probe wider.
                const float probe_limit = estimated_width < initial_width
                    ? initial_accepted_candidates_
                    : static_cast<float>(max_width_ - 1);
                accepted_candidates_ema_ = std::min(
                    probe_limit, accepted_candidates_ema_ + full_accept_probe_);
