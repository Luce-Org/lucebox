...
public:
    ...
    // True iff chunks [0, ceil(n_tok/chunk_tokens)) are all resident in their
    // natural positions — i.e. the first n_tok tokens of KV are fully covered by
    // the pool with no paging. Used by backends to decide if a prefix is
    // bit-identical to a flat cache.
