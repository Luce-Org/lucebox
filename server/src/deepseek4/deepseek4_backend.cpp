...
static int attention_tp_peer_groups() {
    const char * value = std::getenv("DFLASH_DS4_ATTENTION_TP_GROUPS");
    if (!value || !*value) return 0;
    char * end = nullptr;
    const long groups = std::strtol(value, &end, 10);
    if (end == value || *end != '\0' || groups < 0 ||
...
bool DeepSeek4Backend::compute_uniform_hybrid_placement(const DeepSeek4Weights & w,
    ...
    const bool phase_aware_placement = decode_profile_path &&
        *decode_profile_path;
    const bool critical_path_placement = !all_cold &&
        env_flag_enabled("DFLASH_DS4_TP_CRITICAL_PATH_PLACEMENT");
    if (critical_path_placement) {
...
bool DeepSeek4Backend::init_hybrid_model() {
    ...
    const int attention_peer_groups = attention_tp_peer_groups();
    if (attention_peer_groups < 0) {
        std::fprintf(stderr,
                     "[deepseek4-attention-tp] invalid "
                     "DFLASH_DS4_ATTENTION_TP_GROUPS\n");
        return false;
    }
    if (attention_peer_groups > 0) {
        if (!inprocess_tp || !expert_backend_ ||
            !env_flag_enabled("DFLASH_DS4_FUSED_VERIFY")) {
            std::fprintf(stderr,
                         "[deepseek4-attention-tp] requires in-process "
                         "heterogeneous fused verification\n");
            return false;
        }
        if (!init_deepseek4_attention_tp(
                w_, expert_backend_, attention_peer_groups, &err)) {
            std::fprintf(stderr,
                         "[deepseek4-attention-tp] initialization failed: %s\n",
                         err.c_str());
            return false;
        }
    }
...
bool DeepSeek4Backend::park(ParkTarget target) {
    ...
    moe_hybrid_.reset();
    // Attention-TP weights are allocated by expert_backend_. Release them
    // before destroying that borrowed backend.
    free_deepseek4_weights(w_);
    if (expert_backend_) {
        ggml_backend_free(expert_backend_);
        expert_backend_ = nullptr;
