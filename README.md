...
| **DeepSeek V4 Flash 0731 adaptive ROCmFPX** | — | `DFLASH_DS4_SPEC=1`<br>`DFLASH_DS4_SPEC_Q=4`<br>`DFLASH_DS4_FUSED_VERIFY=1`<br>`DFLASH_DS4_SPARSE_DECODE_FLASH=1`<br>`DFLASH_DS4_DRAFT=<path>`<br>`DFLASH_DS4_DRAFT_GPU=0`<br>`--target-device hip:0`<br>`--ds4-fused-decode`<br>`--ds4-fused-verify-f16-kv`<br>`--ds4-expert-top-k 6`<br>`--ds4-prefill sparse`<br>`--prefill-compression auto`<br>`--prefill-drafter <Qwen3-0.6B-BF16.gguf>`<br>`--prefill-skip-park` | `HIP_VISIBLE_DEVICES=<r9700-index>`<br>`DFLASH_DS4_SPEC=1`<br>`DFLASH_DS4_SPEC_Q=4`<br>`DFLASH_DS4_FUSED_VERIFY=1`<br>`DFLASH_DS4_SPARSE_DECODE_FLASH=1`<br>`DFLASH_DS4_DRAFT=<path>`<br>`DFLASH_DS4_DRAFT_GPU=0`<br>`--target-device hip:0`<br>`--ds4-fused-decode`<br>`--ds4-fused-verify-f16-kv`<br>`--ds4-expert-top-k 6`<br>`--ds4-prefill sparse`<br>`--prefill-compression auto`<br>`--prefill-drafter <Qwen3-0.6B-BF16.gguf>` |
...
DeepSeek4 sparse prefill and PFlash trade verbatim prompt ingestion for speed.
Use `--ds4-prefill exact --prefill-compression off` when the application needs
reference-exact ingestion; compare true-context decode with PFlash disabled.
