...
ROOT = HOME / 'lucebox-ds4v-vision-unbiased'
BUILD = Path('/tmp/ds4v-unbiased-hip-build')
SOURCE = '53bf07d8d327f93fc4a4f479220023907154cb3a'
...
FIXED = {
    BUILD / 'ds4v_vision_probe': 'd0e610716acd357cf295013d018caa26066d520722a9b35f21ba60013d5b36cc',
...
LIBRARIES = {
    'libggml-base.so.0': (BUILD / 'ggml/src/libggml-base.so.0', 'ddd61d98d11d5209466e74a803c421079dbddba5dcb92d2acb6c638d55b78278'),
    'libggml-cpu.so.0': (BUILD / 'ggml/src/libggml-cpu.so.0', '744352d31cc70b3f4afcbf17f56f49399056f80b493748f65e05de556572bfe9'),
    'libggml-hip.so.0': (BUILD / 'ggml/src/ggml-hip/libggml-hip.so.0', '17d1f73ffeb9ac6701bc5662b6a888bc35a9545c2e6c8025e46117eb7ad4aa49'),
    'libggml.so.0': (BUILD / 'ggml/src/libggml.so.0', 'a18bc2a1adbebc448c652e79b3e97862fd1b09926044025e339ab951a515ae68'),
...
def main():
    ...
    require(production.get('schema') == 'ds4v-unbiased-production-runtime-v1'
    ...
    require(linear.get('schema') == 'ds4v-unbiased-native-linear-proof-v1' and linear.get('pass') is True
    ...
    require([x['name'] for x in linear['lanes']] == ['tiny', 'patch', 'qkv', 'mlp_w1', 'mlp_w2'], 'five production linear lanes required')
    for lane in linear['lanes']:
        ...
        expected_dispatch = 1 if lane['name'] in ('mlp_w1', 'mlp_w2') else 2
    ...
    report = {'pass': False, 'scope': 'concurrent numerical-only; CPU portability separate; no HTTP/performance acceptance',
              ...
              'reference': str(REFERENCE), 'policy_sha256': pins[policy], 'linear_receipt': cfg['linear_receipt'],
              'production_pins': cfg['production_pins'], 'lanes': []}
    try:
