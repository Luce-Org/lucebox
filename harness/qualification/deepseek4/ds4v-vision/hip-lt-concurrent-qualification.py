...
SOURCE = '6137f4305400247fed98d2144634c184e2bc6b13'
...
FIXED = {
    BUILD / 'ds4v_vision_probe': '635a49b45d116a62d405898230389f41e77aa51d47437c02ffa647919fd458d7',
...
LIBRARIES = {
    ...
    'libggml.so.0': (BUILD / 'ggml/src/libggml.so.0', 'd71378079c9ea008269964b34e9aa48a06db703e6c69db92b1aa7c803e5eee72'),
...
def main():
    ...
    require(linear['source_commit'] == SOURCE and linear['red_commit'] == '983be861d878681a26f9f8c9e8cd4f804ab0f14d', 'linear source mismatch')
    require(linear['pins_sha256'] == '0b2fb11d1b8d2b39716d9e19a3c61f4340ec21c7e15b3242e11f7d4ccfe660e5', 'linear pins mismatch')
    ...
    verify_pins(pins)
    require(set(cfg['guard_pins']) == {'guard.py', 'host.py', 'receipt.py', 'run.py'}, 'guard pin set incomplete')
    ...
    for lane in cfg['lanes']:
        ...
        out = HOME / 'ds4v-work/radeon-numerical-guard' / p['run_name']
    ...
    try:
        for lane, (p, out, label) in zip(cfg['lanes'], policies):
            ...
            report['lanes'].append({'name': lane['name'], 'policy_path': lane['policy'], 'policy_sha256': lane['sha256'],
                                    ...
                                    'command': p['command'], 'child_exit': result['exit'], 'outputs': result['output_evidence']})
