...
ROOT = HOME / 'lucebox-ds4v-vision-norm'
BUILD = Path('/tmp/ds4v-norm-hip-build')
SOURCE = 'ed661d01a5dfebb02009f23816aec18f8cc87178'
...
LANES = ['tiny', 'patch', 'qkv', 'mlp_w1', 'mlp_w2', 'norm782', 'norm2562']
FIXED = {
    SUPERVISOR: SUPERVISOR_SHA,
    BUILD / 'ds4v_vision_probe': '75215a951ca640e250c040e31010bb202723da89774f992566525e8c56f9c332',
...
LIBRARIES = {
    'libggml-base.so.0': (BUILD / 'ggml/src/libggml-base.so.0', 'bdaf7f896e931898241e2f77775511d1a959af9293e0eb8c2c3d2c323095a253'),
    'libggml-cpu.so.0': (BUILD / 'ggml/src/libggml-cpu.so.0', '1622da3087c042dcd5bcf27487890a8301a9baaa4f57fe11c98c40b1898e6e10'),
    'libggml-hip.so.0': (BUILD / 'ggml/src/ggml-hip/libggml-hip.so.0', 'bb296f9dd83d5ea5a9dbf844e02e8ac426358d7035fb9db1642d6a8f63eae462'),
    'libggml.so.0': (BUILD / 'ggml/src/libggml.so.0', 'e0e2a256cdeac6139f779a417ae11fed414e92b43781c16a0835c003054d680c'),
...
def main():
    ...
    require(production.get('schema') == 'ds4v-norm-production-runtime-v1'
    ...
    require(linear.get('schema') == 'ds4v-norm-native-production-proof-v1' and linear.get('pass') is True
    ...
    require([x['name'] for x in linear['lanes']] == LANES, 'seven production prerequisite lanes required')
    ...
    for lane in linear['lanes']:
        ...
        expected_dispatch = 0 if norm else 1 if lane['name'].startswith('mlp_') else 2
