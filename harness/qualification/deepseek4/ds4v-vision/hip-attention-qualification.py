...
ROOT = HOME / 'lucebox-ds4v-vision-attention'
BUILD = Path('/tmp/ds4v-attention-hip-build')
SOURCE = '686285f092c961423747a8c961a509767aec1017'
...
SUPERVISOR = HOME / 'ds4v-work/hipblaslt-stage-diagnostic/log-snapshot-guard.py'
SUPERVISOR_SHA = '8591d28b0b11a1531fe2a657080958cbf401fda9a19dd18d747c09e5edbd06cb'
LANES = ['tiny', 'patch', 'qkv', 'mlp_w1', 'mlp_w2', 'norm782', 'norm2562', 'rotary', 'softmax', 'attention']
FIXED = {
    SUPERVISOR: SUPERVISOR_SHA,
    BUILD / 'ds4v_vision_probe': 'd9ad23010e8dae30623442b94697587ca7c160824b2fe5c48ddaa6f6a380949d',
...
LIBRARIES = {
    'libggml-base.so.0': (BUILD / 'ggml/src/libggml-base.so.0', 'e14f36e6e2ad059e404c658d307b03a5c20cfbf57ea3ded00049ee872e080f86'),
    'libggml-cpu.so.0': (BUILD / 'ggml/src/libggml-cpu.so.0', '91d2d975096a3a4597f033ee2250d0df092be1493e212d9bf84ce8099c443590'),
    'libggml-hip.so.0': (BUILD / 'ggml/src/ggml-hip/libggml-hip.so.0', '67debd0e1638230a6f2c94690483b660bff9e071e41eeef61f35d386cdfcc817'),
    'libggml.so.0': (BUILD / 'ggml/src/libggml.so.0', '60cc08a311f0ad9121a8760adac132c46cd028bb8070acd55632e50c89dee397'),
...
def verify_pins(pins):
    for path, sha in pins.items():
        require(re.fullmatch('[0-9a-f]{64}', sha) is not None, 'unbound or invalid pin: ' + str(path))
...
def attention_counts(name):
    softmax = 6 if name == 'softmax' else 1 if name == 'attention' else 0
    av = 1 if name == 'attention' else 0
    return {'explicit_softmax_ops': softmax, 'actual_softmax_launches': softmax,
            'explicit_av_ops': av, 'actual_av_launches': av,
            'actual_rotary_launches': 1 if name in ('rotary', 'attention') else 0}
...
def main():
    ...
    require(re.fullmatch('[0-9a-f]{40}', SOURCE) is not None, 'source commit is not bound')
    ...
    production_path = Path(cfg['production_pins']['path'])
    production_sha = cfg['production_pins']['sha256']
    require(re.fullmatch('[0-9a-f]{64}', production_sha) is not None
            and digest(production_path) == production_sha, 'production pin manifest changed')
    production = json.loads(production_path.read_text())
    require(production.get('schema') == 'ds4v-attention-production-runtime-v1'
            and production.get('source_root') == str(ROOT)
            and production.get('source_commit') == SOURCE and isinstance(production.get('files'), dict)
            and production['files'], 'production source/file pins missing')
    linear_path = Path(cfg['production_receipt']['path'])
    require(digest(linear_path) == cfg['production_receipt']['sha256'], 'linear acceptance receipt changed')
    linear = json.loads(linear_path.read_text())
    require(linear.get('schema') == 'ds4v-attention-native-production-proof-v1' and linear.get('pass') is True
            and linear.get('scope') == 'numerical-only-concurrent', 'accepted linear numerical proof required')
    require(linear['source_commit'] == SOURCE, 'linear source mismatch')
    require(linear['pins_sha256'] == production_sha, 'linear production pins mismatch')
    require([x['name'] for x in linear['lanes']] == LANES, 'ten production prerequisite lanes required')
    require(linear['supervisor'] == production['supervisor'] == {'path': str(SUPERVISOR), 'sha256': SUPERVISOR_SHA},
            'logging supervisor differs from accepted prerequisites')
    require(linear['source_norm'] == production['source_norm'], 'normalization source provenance mismatch')
    require(linear['source_attention'] == production['source_attention'], 'attention source provenance mismatch')
    require(linear['source_acceptance'] == production['source_acceptance'] == {
        'path': str(SUPERVISOR.parent / 'unbiased-source-acceptance-v1.json'),
        'sha256': '1d0d59f2b3ffc21a366df50ef7257fe6ac2a1eca78cbea10988df3f2398bdbfa'}, 'source acceptance changed')
    for lane in linear['lanes']:
        require(lane['guard_exit'] == 0 and lane['child_exit'] == 0
                and lane['component_numeric_pass'] is True, 'linear lane not accepted')
        require(lane['source_bitwise_mismatches'] == 0, 'production linear differs from frozen source')
        norm = lane['name'].startswith('norm')
        expected_dispatch = 0 if norm or lane['name'] in ('rotary', 'softmax', 'attention') else 1 if lane['name'].startswith('mlp_') else 2
        require(lane['explicit_ops'] == lane['actual_lt_launches'] == expected_dispatch,
                'wrong production linear dispatch count')
        require(lane['explicit_norm_ops'] == lane['actual_norm_launches'] == (1 if norm else 0),
                'wrong production normalization dispatch count')
        for field, expected in attention_counts(lane['name']).items():
            require(lane[field] == expected, 'wrong production attention dispatch count: ' + field)
        if norm:
            require(lane['reference_layout'] == ('original' if lane['name'] == 'norm782' else 'rowwise-tiled-original'),
                    'normalization reference layout changed')
    ...
    pins[linear_path] = cfg['production_receipt']['sha256']
    pins[production_path] = production_sha
    pins.update({path: sha for path, sha in LIBRARIES.values()})
    for path, sha in production['files'].items():
        require(Path(path).is_absolute(), 'absolute production file pin required')
        require(Path(path) not in pins or pins[Path(path)] == sha, 'production pin conflicts with qualification pin: ' + path)
        pins[Path(path)] = sha
    for path, sha in linear['input_artifact_hashes'].items():
        require(Path(path).is_absolute() and (Path(path) not in pins or pins[Path(path)] == sha),
                'prerequisite provenance conflicts with qualification pins')
        pins[Path(path)] = sha
    for lane in linear['lanes']:
        pins[Path(lane['guard_report_path'])] = lane['guard_report_sha256']
    ...
    verify_pins(pins)
    for path, sha in json.loads(runtime.read_text())['files'].items():
        require(Path(path) not in pins or pins[Path(path)] == sha, 'comparator runtime pin conflict')
        pins[Path(path)] = sha
    ...
    for lane in cfg['lanes']:
        ...
        require('hip_vision_linear_launches=131 retained_workspace_bytes=79691776' in p['required_log_lines'], 'dispatch contract missing')
        require('hip_vision_norm_launches=65' in p['required_log_lines'], 'normalization dispatch contract missing')
        for name, count in (('rotary', 1), ('softmax', 32), ('av', 32)):
            require('hip_vision_' + name + '_launches=' + str(count) in p['required_log_lines'],
                    'attention dispatch contract missing: ' + name)
        require(p['component_pins'].get(str(SUPERVISOR)) == SUPERVISOR_SHA, 'logging supervisor is not pinned')
    ...
    require(len({str(out) for _, out, _ in policies}) == 3, 'full lane evidence directories must be distinct')
    ...
    report = {'pass': False, 'scope': 'concurrent numerical-only; CPU portability separate; no HTTP/performance acceptance',
              ...
              'reference': str(REFERENCE), 'policy_sha256': pins[policy], 'production_receipt': cfg['production_receipt'],
              'production_pins': cfg['production_pins'], 'supervisor': production['supervisor'], 'lanes': []}
    try:
        for lane, (p, out, label) in zip(cfg['lanes'], policies):
            verify_pins(pins)
            supervisor_log = evidence / (lane['name'] + '-supervisor.log')
            with supervisor_log.open('x') as log:
                guarded_run([str(PYTHON), '-I', '-B', str(SUPERVISOR), '--policy', lane['policy'],
                             '--policy-sha', lane['sha256'], '--parent-radeon-window-released'], log)
            pins[supervisor_log] = digest(supervisor_log)
            ...
            require(re.findall(r'^hip_vision_linear_launches=(\d+) retained_workspace_bytes=(\d+)$', log, re.M) == [('131', '79691776')], 'ambiguous/incomplete full-tower dispatch')
            require(re.findall(r'^hip_vision_norm_launches=(\d+)$', log, re.M) == ['65'],
                    'ambiguous/incomplete full-tower normalization dispatch')
            for name, count in (('rotary', 1), ('softmax', 32), ('av', 32)):
                require(re.findall(r'^hip_vision_' + name + r'_launches=(\d+)$', log, re.M) == [str(count)],
                        'ambiguous/incomplete full-tower attention dispatch: ' + name)
            ...
            report['lanes'].append({'name': lane['name'], 'policy_path': lane['policy'], 'policy_sha256': lane['sha256'],
                                    ...
                                    'actual_lt_launches': 131, 'actual_norm_launches': 65,
                                    'actual_rotary_launches': 1, 'actual_softmax_launches': 32, 'actual_av_launches': 32,
                                    'supervisor_log_path': str(supervisor_log), 'supervisor_log_sha256': pins[supervisor_log],
