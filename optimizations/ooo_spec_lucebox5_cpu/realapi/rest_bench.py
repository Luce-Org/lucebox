...
def run_task(task_id: str, prompt: str, expect: list[str], arm: str, single_call: bool) -> dict:
    ...
    for turn_index in range(MAX_TURNS):
        ...
        early_hits = {e["result"]["call_sha256"]: e["result"] for e in early if e.get("status") == "hit" and isinstance(e.get("result"), dict) and e["result"].get("ok") is True}
        early_by_id = {e["call_id"]: e["result"] for e in early if e.get("status") == "hit" and e.get("call_id") and isinstance(e.get("result"), dict) and e["result"].get("ok") is True}
        for i, (cid, name, args) in enumerate(parsed):
            if results[i] is None and args is not None and name in rest_tools.TOOL_NAMES:
                ...
                if cid in early_by_id:
                    results[i] = early_by_id[cid]; sources[i] = "early_dispatch_hit"; hits += 1
                elif sha in early_hits:
        ...
        for i, (cid, name, args) in enumerate(parsed):
            answer_results.append(results[i])
    ...
    return {"task": task_id, "arm": arm, "prompt": prompt, "final": final, "correct": correct, "wall_ms": wall_ms, "model_ms": model_ms,
            ...
            "multi_call_turns": multi_call_turns, "spec_statuses": spec_statuses, "validation": validation, "turn_log": turns}
...
def main() -> None:
    ...
    out_path = results_dir / f"rest_{tag}.json"
    results = []
    for idx, (tid, prompt, expect) in enumerate(TASKS[:n]):
        ...
        for arm in order:
            ...
            with out_path.open("w", encoding="utf-8") as output:
                json.dump({"tag": tag, "single_call": single_call, "results": results}, output, indent=1)
    # summary
    by = {}
    for r in results:
    ...
    if ratios and s_tot > 0:
        ...
        print("pairs=0 (no complete non-zero control/spec pairs)")
