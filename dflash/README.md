...
`GGML_CUDA_NO_VMM` is a **compile-time** CMake option — it cannot be set at runtime via environment variable. A follow-up PR adds a runtime warning to the dflash test binary when it detects <=24 GiB VRAM and was built without this flag.
