...
run` plumbing, systemd integration, and isolated profiles for already-installed
AI clients. This package owns: TOML config, the host-derived DFLASH_* serve
heuristic, docker daemon calls (via the mounted socket), and model download.
Bounded calibration measures the resolved plan without duplicating the
engine's own adaptive policies.
"""
